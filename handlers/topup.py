...
from database import get_or_create_user, add_payment
from freekassa import create_payment_url, get_callback_url
from config import FREKASSA_SHOP_ID, PUBLIC_BASE_URL
...
async def topup_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(text, parse_mode="Markdown", reply_markup=topup_keyboard())
    await cb.answer()

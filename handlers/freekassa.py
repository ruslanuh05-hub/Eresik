...
logger = logging.getLogger("jvpn-bot.freekassa")
    ...
    try:
        async with httpx.AsyncClient(timeout=12.0) as client:
            ...
            if resp.status_code != 200:
                safe_payload = {k: v for k, v in payload.items() if k != "signature"}
                logger.warning(
                    "FreeKassa API create order failed: status=%s body=%s payload=%s",
                    resp.status_code,
                    (resp.text or "")[:600],
                    safe_payload,
                )
